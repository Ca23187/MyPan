...
const suspendRouteSync = ref(false);
...
watch(
  ...
  async (newVal, oldVal) => {
    ...
    if (suspendRouteSync.value) return; // ✅ 新增：外部强制设置面包屑时，不走路由驱动逻辑
    if (
...
// ✅ 搜索跳转：一次性设置完整面包屑链
const setBreadcrumb = async (list) => {
  suspendRouteSync.value = true;
  try {
    folderList.value = (list || []).map((x) => ({
      fileId: x.fileId,
      fileName: x.fileName,
    }));
    ...
    const path = folderList.value.map(x => x.fileId).join("/");
    await router.replace({
      path: route.path,
      query: path ? { path } : {},
    });
  } finally {
    // 放到微任务后再打开，确保这次 replace 引发的 watch 不再执行
    Promise.resolve().then(() => {
      suspendRouteSync.value = false;
    });
  }
};


// ✅ 一定要暴露出去，否则 Main 里的 setBreadcrumb 调不到
defineExpose({ openFolder, setBreadcrumb });
</script>
