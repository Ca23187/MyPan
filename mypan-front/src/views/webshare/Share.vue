        ...
        <div class="share-panel">
          <div class="share-user-info">
            ...
            <div class="share-info">
              <div class="user-info">
                ...
                <span class="share-time">Shared on {{ shareInfo.sharedOn }}</span>
              </div>
          ...
          <div class="share-op-btn">
            ...
            <el-button
              ...
              @click="cancelShare"
              ><span class="iconfont icon-cancel"></span>&nbsp;Stop Sharing</el-button
            >
            <el-button
              type="primary"
              v-else
              ...
              >&nbsp;Save to My Drive</el-button
            >
            <el-button @click="goMyPan">
              <span class="iconfont icon-pan"></span>
              &nbsp;Back to My Drive
            </el-button>
          </div>
            ...
            <template #fileName="{ index, row }">
              ...
              >
                ...
                >
                  ...
                  <Icon :cover="row.fileCover" :shareId="shareId"></Icon>
                </template>
                <template v-else>
                ...
                <span class="file-name clickable" :title="row.fileName">
                ...
                <span class="op">
                  ...
                  <span
                    ...
                    @click="downloadSingle(row.fileId)"
                    >Download</span
                  >
...
const getShareInfo = async () => {
  ...
  if (!result || result.data == null) {
    ...
    router.replace({
      ...
      query: qCode ? { code: qCode } : {},
    });
    return;
  }

  shareInfo.value = result.data;
  loadDataList(); // ✅ 触发首次加载列表
};
getShareInfo();
...
// 下载文件
const downloadSingle = async (fileId) => {
  const result = await proxy.Request({
    url: `${api.createDownloadUrl}/${shareId}`,
...
const save2MyPanFileIdList = ref([]);
...
const save2MyPan = () => {
  ...
  save2MyPanFileIdList.value = [...selectFileIdList.value];
...
const save2MyPanSingle = (row) => {
  ...
  save2MyPanFileIdList.value = [row.fileId];
  folderSelectRef.value.showFolderDialog();
};
...
const save2MyPanDone = async (folderId) => {
  let result = await proxy.Request({
    ...
    params: {
      shareId: shareId,
      shareFileIds: save2MyPanFileIdList.value.join(","),
      myFolderId: folderId,
    },
  });
  if (!result) {
...
const cancelShare = () => {
  proxy.Confirm(`Are you sure you want to stop sharing?`, async () => {
    ...
    proxy.Message.success("Sharing has been canceled");
    router.push("/");
  });
};
